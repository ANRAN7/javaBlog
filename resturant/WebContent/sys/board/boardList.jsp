    ...
    <table class="MainArea_Content" cellspacing="0" cellpadding="0">
        ...
        <c:forEach items="${requestScope.list }" var="table" varStatus="vs">
          ...
          <td>
            <c:choose>
              <c:when test="${table.tableStatus==1 }">
              <a href="${pageContext.request.contextPath }/table?method=update&id=${table.id }" class="FunctionButtion">退桌</a>
              </c:when>
             <c:when test="${table.tableStatus==0}">
              <a href="${pageContext.request.contextPath }/table?method=update&id=${table.id }" class="FunctionButtion">预定</a>
              </c:when>
              </c:choose>
              <a href="${pageContext.request.contextPath }/table?method=delete&id=${table.id }" onClick="return delconfirm();" class="FunctionButtion">删除</a>
          </td>
        </tr>
        </c:forEach>
        </tbody>
